 ...
 * @returns
 */
export const delayed = async (value, ms = 1000) => new Promise((resolve) => {
    setTimeout(() => {
        resolve(value);
    }, ms);
});
 ...
 * @returns
 */
export function dispatch(v, f, onRejection = (error) => Promise.reject(error)) {
 ...
 */
export function dispatchPromiseOrValueArray(a, f, onRejection = (error) => Promise.reject(error)) {
    const indexPromise = a.findIndex((v) => v instanceof Promise);
    if (indexPromise !== -1) {
        // if one deps is a promise, make a promise of them all
        const all = Promise.all(a.map((v) => (v instanceof Promise ? v : Promise.resolve(v))));
