...
test("cell pointer async", async () => {
    ...
    const cell1 = proxy.new(delayed(1, 100), "init");
    const cellMap = cell1.map((x) => proxy.new(x + 2), "cell1");
    const mixed = proxy.map([cell1, cellMap], async (a, b) => delayed(a + b, 150), "mixed");
    await expect(cellMap.get()).resolves.toEqual(3);
    expect(cellMap.isPointer).toEqual(true);
    await expect(mixed.get()).resolves.toEqual(4);
    cell1.set(2);
...
test("cell pointer async initially undefined", async () => {
    const sheet = new Sheet();
    const proxy = new SheetProxy(sheet);
    const cell1 = proxy.new(undefined, "init");
    const cellMap = cell1.map((x) => proxy.new(x + 2), "cell1");
    cell1.set(1);
    expect(cellMap.isPointer).toEqual(true);
    await expect(cellMap.get()).resolves.toEqual(3);
...
test("cell pointer chain", async () => {
    ...
    const cell1 = proxy.new(delayed(1, 100), "init");
    const cellMap = cell1.map((x) => proxy.new(x + 2), "cell1");
    const mixed = proxy.map([cell1, cellMap], async (a, b) => delayed(proxy.new(a + b), 150), "mixed");
    await expect(cellMap.get()).resolves.toEqual(3);
    expect(cellMap.isPointer).toEqual(true);
    await expect(mixed.get()).resolves.toEqual(4);
    cell1.set(2);
...
test("cell pointer longer chain", async () => {
    ...
    const cellMap = init.map((x) => proxy.new(x + 2, `cellMap_${x}`), "cellMap", true);
    const mixed = proxy.map([init, cellMap], async (a, b) => delayed(proxy.new(a + b, `mixed_${a}_${b}`), 150), "mixed", true);
    const successiveValues = [];
    mixed.subscribe((v) => successiveValues.push(v));
    const double = mixed.map(async (v) => proxy.new(v * 2, `double_${v}`), "double");
    await expect(cellMap.get()).resolves.toEqual(3);
    expect(cellMap.isPointer).toEqual(true);
    await expect(mixed.get()).resolves.toEqual(4);
...
test("cell pointer, incremental computation", async () => {
    const sheet = new Sheet();
    const proxy = new SheetProxy(sheet);
    const init1 = proxy.new(1, "init1");
    const cellPointer1 = proxy.new(init1, "pointer1");
    ...
    sheet.selectUpdatableCells(set, fn);
    const { updatable, pointersToBeUpdated, grey, mightChange: toBeUpdated } = select(new Set([init1.id]), (id) => sheet.get(id).isPointer);
...
test("cell pointer, update pointed", async () => {
    ...
    const depOnPointer = proxy.map([cellPointer1], async (a) => delayed(a + 1, 150), "depOnPointer");
    await expect(depOnPointer.get()).resolves.toEqual(2);
    init1.set(2);
...
test("cell pointer long chain, update pointed", async () => {
    ...
    const depOnPointer = proxy.map([cellPointer4], async (a) => delayed(a + 1, 1), "depOnPointer");
...
test("cell pointer long chain, update pointed with null", async () => {
    ...
    const cellPointer3 = proxy.new(delayed(cellPointer2, 15), "pointer3");
    const cellPointer4 = proxy.new(delayed(cellPointer3, 15), "pointer4");
    const depOnPointer = proxy.map([cellPointer4], async (a) => delayed(a, 150), "depOnPointer");
...
test("cell pointer long chain, update pointer", async () => {
    ...
    const cellPointer1 = proxy.new(delayed(init1, 15), "pointer1");
    const cellPointer2 = proxy.new(delayed(cellPointer1, 15), "pointer2");
    const depOnPointer = proxy.map([cellPointer2], async (a) => delayed(a + 1, 150), "depOnPointer", true);
    const valuesOfDepOnPointer = [];
