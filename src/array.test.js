...
import { filter, filterPredicateCell, find, findIndex, first, last, mapArray, mapArrayCell, mapArrayRec, reduce, sort } from "./array";
...
test("mapArray", async () => {
    ...
    const m = mapArray(proxy, l, (v) => v + 1);
...
test("mapArrayCell function change", async () => {
    ...
    const l = proxy.new([1, 2, 3].map((v) => proxy.new(v)));
    expect(sheet.stats.count).toBe(4);
    const fn = proxy.new((v) => v.map((v) => v + 1));
...
test("mapArray async", async () => {
    ...
    const l = proxy.new([1, 2, 3].map((v) => proxy.new(v)));
    expect(sheet.stats.count).toBe(4);
    const m = mapArray(proxy, l, (v) => delayed(v + 1, 10));
    expect(sheet.stats.count).toBe(8); // +4 cells
    // initial value
    await expect(_uncellify(m)).resolves.toEqual([2, 3, 4]);
    expect(sheet.stats.count).toBe(8); // unchanged
...
test("filterPredicateCell array", async () => {
    ...
    const pred = proxy.new((v) => v.map((_v) => _v > 2));
...
test("sort array remapped", async () => {
    ...
    const sum = reduce(proxy, sp, async (acc, v) => (await acc) + v, Promise.resolve(0));
    await expect(_uncellify(s)).resolves.toEqual([1, 3, 5]);
...
test("mapArrayRec array", async () => {
    const sheet = new Sheet();
    const proxy = new SheetProxy(sheet);
    const arr = _cellify(proxy, [1, 2, [3, [4]]]);
