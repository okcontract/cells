...
const unwrappedCell = (proxy, qCell, name) => {
    const cell = proxy.new(undefined, name);
    let uns;
    const data = proxy.map([qCell], (_q) => (_q && new WrappedCell(proxy.new(delayed({ data: _q }, 1000)), proxy)) ||
        null);
    data.subscribe((v) => {
...
test("unwrappedCell in a class and wait for value", async () => {
    ...
    await expect(testClass.mapped.consolidatedValue).resolves.toHaveProperty("data");
});
