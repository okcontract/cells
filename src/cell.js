 ...
 */
export class Cell extends SubscribeBench {
    constructor(sheet, id, dependencies = undefined, options = {
        noFail: false
    }) {
        super();
        ...
        this._valueRank = 0;
        this._currentComputationRank = 0;
        this._pendingRank = null;
        this._pendingResolve = (_) => { };
         ...
         * @returns mapped cell
         */
        this.map = (fn, name, noFail) => this._sheet instanceof SheetProxy
            ? this._sheet.map([this], fn, name, noFail)
            : this._sheet.map([this], fn, name, undefined, noFail);
        this.init = (fn, name) => {
            const getter = async () => {
            ...
            const cell = this._sheet instanceof SheetProxy
                ? this._sheet.new(getter(), name)
                : this._sheet.new(getter(), undefined, name);
         ...
         */
        this.get = async () => this.value !== undefined
            ? this.value
    ...
    get sheet() {
        if (this._sheet instanceof SheetProxy)
            return this._sheet._sheet;
        return this._sheet;
    }
    /**
    ...
    setPointed(newPointed) {
        ...
        if (newPointed !== this._pointed) {
            this.sheet._updatePointer(this.id, this._pointed, newPointed === null ? undefined : newPointed);
            this._isPointer = true;
     ...
     */
    setPendingComputation(rank, computation) {
        ...
        if (this._pendingRank === null || this._pending_ === undefined) {
            this._pending_ = new Promise((resolver, _rejecter) => {
    ...
    get consolidatedValue() {
        ...
        const pending = this._pending_;
        return pending.then((v) => v instanceof Canceled
    ...
    get consolidatedValueWthUndefined() {
        ...
        if (pending === undefined) {
            console.error("Pending undefined although compRak differs from value rank", this.sheet.naming({
                cell: this.id,
                valueRank: this._valueRank,
        ...
        return pending.then((v) => this.isPointer
    ...
    get consolidatedError() {
        ...
        return dispatch(this.consolidatedValue, (value) => this.v instanceof Error ? this.v : undefined);
    }
     ...
     */
    _setValueOnComputationCompletion(newValue, computationRank, update, skipSubscribers = false) {
        DEV &&
        ...
        if (computationRank < this._valueRank) {
            DEV &&
                console.log(`Cell ${this.name}: `, `setting to ${newValue} has been invalidated`, {
        ...
        if (this.v instanceof Error && !(this.v instanceof CellError)) {
            ...
            if (this._lastStateIsError)
                this._sheet.errors._unsetCellError(this.id);
        }
     ...
     */
    newError(reason, source = undefined) {
        if (source === undefined)
            return reason;
...
export class ValueCell extends Cell {
    constructor(sheet, id, orig = undefined, options) {
        super(sheet, id);
        /**
        ...
        this.update = (fn) => {
            ...
            const v = this.v;
            if (v === undefined)
                throw this.newError("Cell not initialized");
            if (v instanceof Error)
                throw this.newError("Cell has error");
            if (this._currentComputationRank !== this._valueRank) {
                throw this.newError(`Cell's value is being concurrently computed, cannot update`);
            }
            const nv = fn(v);
            this.set(nv);
        };
        ...
        this.apply = (fn) => {
            // this line is here to help the typechecker
            if (this.isPointer)
                throw this.newError("Cell is a pointer");
            //@ts-expect-error isPointer rules out AnyCell
            const v = this.v;
            if (v === undefined)
                throw this.newError("Cell not initialized");
    ...
    set(value) {
        ...
        if (value instanceof Promise) {
            ...
            const computation = value.then((v) => 
            // console.log({ setPromisedInCell: this.name, resolvedToValue: v });
            this._setValueOnComputationCompletion(v, computationRank, true, true), (error) => this._setValueOnComputationCompletion(error, computationRank, true, true));
...
export class MapCell extends Cell {
     ...
     */
    dependencies, usePreviousValue, noFail, // noFail indicates the computeFn
    isVolatile = false // @todo volatile functions like NOW() must be recomputed all the time
    ) {
        super(sheet, id, dependencies.map((cell) => cell.id), { noFail: noFail || false });
     ...
     * @returns
     */
    _internalCompute(paramsResults, computationRank, forceNotification) {
        // cancel when on dependency is canceled
        const firstCanceled = paramsResults.findIndex((v) => v instanceof Canceled || v === undefined);
        if (firstCanceled >= 0) {
            ...
            this.value, computationRank, false, false);
            DEV &&
                console.log("Cancel Computation on canceled parameter", this.sheet.naming({ paramsResults, firstCanceled, cell: this.id }));
        ...
        if (firstError >= 0) {
            ...
            res, computationRank, false, !forceNotification);
            return res;
        }
        // Errors have been ruled out, so we can safely convert to V[]
        const params = paramsResults;
        ...
        if (this._currentComputationRank === computationRank) {
            ...
            // @todo do we still check for errors in NoFail?
            // @ts-expect-error
            const newValuePromise = newValue instanceof Promise
            ...
            const cancelOnUndefined = (v) => v === undefined ? cancelComputation : v;
            ...
            if (newValue instanceof Promise) {
                ...
                const pendingComputation = newValuePromise.then((v) => {
                    ...
                    v, computationRank, false, !forceNotification);
            ...
            this._setValueOnComputationCompletion(
            // @ts-expect-error @todo Not<NF> differs from MaybeError
            newValue, computationRank, false, !forceNotification);
     ...
     */
    _computeValue(providedDependencies, 
    ...
    forceNotification) {
        if (this._computeFn !== undefined) {
            try {
                ...
                const params = this._gatherDependencies(providedDependencies);
                ...
                dispatch(params, (params) => {
                    ...
                    p instanceof Cell
                        ? // if provided value is a cell, dereference it
                            p.value
                        : p);
                    return this._internalCompute(dereferencedParams, computationRank, forceNotification);
                });
                const pendingComputation = computed instanceof Promise ? computed : Promise.resolve(computed);
                DEV &&
...
export class Working extends SubscribeBench {
    constructor(nextWorkingCell) {
        super();
        this.value = false;
        this.pending = Promise.resolve();
        this._onGoingComputation = Promise.resolve();
        this._onGoingComputationCount = 0;
        this.resolve = () => { };
...
export class CellErrors extends SubscribeBench {
    constructor(nextErrorsCell) {
        super();
        this.value = new Map();
