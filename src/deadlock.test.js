...
test("unwrappedCell in a class and wait for value", async () => {
    ...
    widget.subscribe((_) => console.log({ widget: widget.id, value: widget.value }));
    const testClass = new TestClass(proxy, widget);
    await expect(testClass.contract.consolidatedValue).resolves.toHaveProperty("data");
});
