...
import { Canceled, CellErrors, MapCell, ValueCell, Working, cancelComputation } from "./cell";
...
export class Sheet {
     ...
     */
    constructor(equality = (a, b) => a === b, marshaller = (a) => JSON.stringify(a)) {
        /**
         * for debugging
         */
        this._debug = false;
    ...
    _updatePointer(from, oldPointer, to) {
        ...
        if (oldPointer !== undefined)
            this.unsetPointerDep(from, oldPointer);
        if (to !== undefined)
            this._pointers.addEdge(to, from);
    }
    /** Only public for proxy usage. */
    _addCell(cellBuilder) {
        const id = this[count]++;
        this[size]++;
        this.g.addNode(id);
     ...
     * @returns
     */
    new(value, proxy, name, // @todo replace with options
    options) {
        const cell = this._addCell((id) => new ValueCell(proxy || this, id, 
        // @todo maybe we should call cell.set(value) if not a Promise?
        value instanceof Promise ? undefined : value, options));
        this._debug &&
            console.log({
                newInSheet: cell.id,
                name,
        ...
        if (value instanceof Promise) {
            this.working.addComputation(cell.id, 
            // print a message in the console in case of promise rejection
            dispatch(cell.set(value), (v) => { }, (error) => {
                console.log(`Cell ${cell.id}: promise of value at cell creation as been rejected: ${error}`);
                return Promise.reject(error);
            }));
        }
        if (name !== undefined)
            this.bless(cell.id, name);
    ...
    mapRaw(dependencies, computeFn, usePreviousValue, name, proxy, noFail) {
        ...
        const mapCell = this._addCell((id) => new MapCell(containingSheet, id, computeFn, dependencies, usePreviousValue, noFail));
        if (name)
            mapCell.bless(name);
        // console.log("Sheet.map:", `registered cell[${id}]`)
        for (const cell of dependencies)
            this.g.addEdge(cell.id, mapCell.id);
        return mapCell;
    }
    map(dependencies, computeFn, name, proxy, noFail) {
        return this.mapRaw(dependencies, computeFn, true, name, proxy, noFail);
    }
    mapNoPrevious(dependencies, computeFn, name, proxy, noFail) {
    ...
    _update(ids) {
         ...
         */
        const updateRec = ({ roots, computations, done, canceled }) => {
            if (this._debug) {
                // @todo _watchAll
                const inter = this._logList.filter((v) => roots.has(v));
                (inter.length || DEV) &&
                    console.log(this.naming({ updateRec: roots, done, computations, canceled }));
            }
            if (roots.size === 0) {
                return { roots, computations, done, canceled };
            }
            return dispatch(this.updateIteration(roots, done, canceled, computations), (result) => updateRec({
        ...
        for (const id of roots) {
            computations[id] = dispatch(this.get(id).consolidatedValueWthUndefined, (v) => (v === undefined ? cancelComputation : v));
        }
        return dispatch(
    ...
    registerCancelAndDone(updatable, computations, done, canceled) {
        const maybeDone = new Set();
        ...
        const pointersToCanceled = this._pointers.strictlyReachableProperty(maybeDone, (id) => canceled.has(id), {
    ...
    updateIteration(ids, done, canceled, computations) {
        if (this._debug) {
            const inter = this._logList.filter((v) => ids.has(v));
            (inter.length || DEV) &&
                console.log(this.naming({ updateIterationOn: ids, done, canceled }));
        }
        const isPointer = (id) => this.get(id).isPointer;
        /** List of nodes that will be updated  */
        const selection = this.selectUpdatableCells(ids, isPointer);
        const { toBeRecomputed, updatable, pointersToBeUpdated, grey, mightChange } = selection;
        if (this._debug) {
            const inter = this._logList.filter((v) => ids.has(v));
            (inter.length || DEV) &&
                console.log("selectUpdatableCells result", this.naming({
                    ...selection,
                    done,
                    canceled
                }));
        ...
        for (const id of toBeRecomputed) {
            if (done.has(id) || canceled.has(id)) {
                console.error("Error - recomputing an already computed cell !:", this.naming({
                    cell: id,
                    ...selection,
                    done,
                    canceled
        ...
        const borderComputation = dispatchPromiseOrValueArray(newComputations, 
        // wait for all new computations to be over before proceeding to the next step
        (newComputations) => {
            // finding all canceled computations
            this.registerCancelAndDone(updatable, newComputations, done, canceled);
            const nextIteration = this.selectBorderUpdatable(ids, updatable, pointersToBeUpdated, grey, isPointer);
            return {
                computationsOfBorder: this.computeUpdatable(nextIteration.toBeRecomputedBorder, computations),
                nextIteration
            };
        });
        //once again, we will wait for the new computations to be over
        const result = dispatch(borderComputation, ({ computationsOfBorder, nextIteration }) => {
            return dispatchPromiseOrValueArray(computationsOfBorder, (computationsOfBorder) => {
                //checking for canceled computations
                this.registerCancelAndDone(nextIteration.safeBorder, computationsOfBorder, done, canceled);
                ...
                if (this._debug) {
                    ...
                    const inter = this._logList.filter((v) => ids.has(v));
                    (inter.length || DEV) &&
                        console.log("Border recomputed, end of iteration:", this.naming(iterationResult));
                }
    ...
    selectBorderUpdatable(roots, updated, pointersToBeUpdated, grey, isPointer) {
         ...
         */
        const greyUpdatedPointers = this._pointers.strictlyReachableProperty(updatedPointers, (id) => grey.includes(id));
        /** List of updated pointers nodes that have been updated adn whose pointed value will not change */
        const greenPointers = updatedPointers.filter((id) => !greyUpdatedPointers.has(id));
        /** candidates for next update iteration */
        const border = Array.from(new Set(greenPointers.flatMap((id) => this.dependentCells(id))));
        /** candidates whose value depends indirectly on a grey value are unsafe */
        const unsafeBorder = this.g.strictlyReachableProperty(border, (id) => grey.includes(id), {
            next: (id) => Array.from(new Set([
        ...
        const toBeRecomputedBorder = safeBorder.filter((id) => this.g
            .predecessors(id)
        ...
        if (this._debug) {
            const inter = this._logList.filter((v) => roots.has(v) || updated.includes(v));
            (inter.length || DEV) &&
     ...
     */
    selectUpdatableCells(ids, isPointer) {
        const next = (id) => this.dependentCells(id);
        const mightChange = this.g
        ...
        const grey = this.g.partialTopologicalSortRootsSet(pointersToBeUpdated, {
            next,
            includeRoots: false
        }) || [];
     ...
     */
    computeUpdatable(toBeRecomputed, computations) {
        ...
        while ((currentCellId = order.pop()) !== undefined) {
            const cell = this._cells[currentCellId];
            if (cell !== undefined) {
                if (this._debug && this._logList.includes(currentCellId)) {
                    console.log("Sheet.computeUpdatable, running computation of:", this.naming({
                        cell: cell.id,
                        computations
                    }));
                }
                const pending = cell instanceof MapCell
                    ? cell._computeValue(computations, false)
                    : cell.consolidatedValueWthUndefined;
                const newComputation = dispatch(pending, (v) => v === undefined ? cancelComputation : v);
                computations[currentCellId] = newComputation;
                newComputations[currentCellId] = newComputation;
            }
            else {
                console.warn(`Computation requested for deleted cell: ${currentCellId}`);
    ...
    canDelete(ids) {
        const deps = new Set();
        for (const id of ids)
            for (const dep of this.g.partialTopologicalSort(id))
                deps.add(dep);
        for (const id of ids)
            deps.delete(id);
        return deps.size === 0;
